package bgm
{
	...
	public class MidiPlayer
	{
		...
		static public function play(url:String, fadeInTime:Number = 0):SMFData
		{
			...
			if (smfData.isAvailable()) 
				_play(smfData);
			else 
				smfData.addEventListener(Event.COMPLETE, _waitAndPlay);
		...
		static private function _playNextData(e:SiONEvent):void
		{
			...
			if (_nextData.isAvailable())
				_play(_nextData);
			else 
				_nextData.addEventListener(Event.COMPLETE, _waitAndPlay);
			_nextData = null;
		}
	}
}