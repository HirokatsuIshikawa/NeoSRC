package scene.talk
{
    ...
    public class TalkViewBase extends CSprite
    {
        ...
        /**初期化イベント実行中*/
        protected var _initialLabelFlg:Boolean = false;
        protected var _initialLoadFunc:Function = null;
        /**会話モード*/
        ...
        protected function setText(ary:Array, label:Object):void
        {
            ...
            initialLabelLoad(startText);
        }
        
        private function startText():void
        {
            _initialLoadFunc = null;
            _loadLine = 0;
        ...
        protected function setContinueText(ary:Array, label:Object):void
        {
            ...
            initialLabelLoad(startContinueText);
        }
        
        private function startContinueText():void
        {
            _initialLoadFunc = null;
            _loadLine = 0;
            clearIf();
            ...
            {
                if (MainController.$.map.battleMapPanel.nowPanelType == BattleMapPanel.PANEL_MAP_TALK)
                {
                    //マップ会話
        ...
        private function initialLabelLoad(func:Function):void
        {
            if (_labelData.hasOwnProperty(MainViewer.INIT_LABEL + ":"))
            {
                ...
                _loadLine = _labelData[MainViewer.INIT_LABEL + ":"];
                setLineCommand();
            }
            else
            {
        ...
        protected function setLineCommand():void
        {
            ...
            var startStr:String = line.substr(0, 1);
            var endStr:String = line.substr(line.length - 1, 1);
            
            //追加インクルード
            if (line.substr(0, 2) === "#<" && line.substr(line.length - 1, 1) === ">")
            {
                if (line.toLowerCase() === "#<loadstart>")
                {
                    _labelData = EventFileLoader.resetLabel(_textData, _labelData);
                    setLineCommand();
                }
                else
                {
                    //追加分のテキストをデータに追加
                    EventFileLoader.loadEveData(line.substring(2, line.length - 1), addEveData, false);
                }
            }
            //ラベル
        ...
        protected function playCommand(line:String, command:Array, param:Object):void
        {
            ...
            case "registgenericunit": 
                ...
                //レベル
                if (!param.hasOwnProperty("lv"))
                {
                    param.lv = 1;
                }
                registGenericUnit(param.name, param.side, param.lv, param.cost);
                setLineCommand();
                break;
        ...
        protected function registGenericUnit(name:String, side:String, lv:int, cost:int):void
        {
            var i:int = 0;
            var findFlg:Boolean = false;
            var dataList:Vector.<GenericUnitData> = MainController.$.model.playerGenericUnitData;
            var data:MasterCharaData = MainController.$.model.getMasterCharaData(name);
            
            if (data == null)
            {
                MainController.$.view.errorMessageEve("汎用ユニット登録：" + "が見つかりません", _loadLine);
            }
            
            if (side == "")
            {
                side = MainController.$.model.playerParam.sideName;
            }
            ...
            if (MainController.$.model.playerParam.sideName === side)
            {
                ...
                {
                    if (dataList[i].compareName(name))
                    {
                        dataList[i] = new GenericUnitData(data, lv, cost);
                        findFlg = true;
                    }
                }
            ...
            //陣営番号取得・ない場合は新陣営作成
            var sideNum:int = MainController.$.map.makeNewSide(side);
            var sideFind:Boolean = false;
            //陣営登録
            for (i = 0; i < MainController.$.map.sideState[sideNum].genericUnitList.length; i++)
            {
                if (MainController.$.map.sideState[sideNum].genericUnitList[i].compareName(name))
                {
                    sideFind = true;
                    MainController.$.map.sideState[sideNum].genericUnitList[i] = new GenericUnitData(data, lv, cost);
                    break;
                }
            }
