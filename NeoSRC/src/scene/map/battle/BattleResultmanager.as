package scene.map.battle
{
	...
	public class BattleResultmanager
	{
		...
		/**スキル使用*/
		public function useSkill(_attackWeaponItem:AttackListItem):void
		{
			var i:int = 0;
			//回復
			if (_attackWeaponItem.skill.heal > 0)
			{
				_attackWeaponItem.target.healHP(_attackWeaponItem.skill.heal);
			}
			//補給
			if (_attackWeaponItem.skill.supply > 0)
			{
				_attackWeaponItem.target.supplyFP(_attackWeaponItem.skill.supply);
			}
			//効果
			if (_attackWeaponItem.skill.buff != null)
			{
				var findFlg:Boolean = false;
				var buffName:String = _attackWeaponItem.skill.buff;
				//該当バフデータを検索
				for (i = 0; i < MainController.$.model.masterBuffData.length; i++)
				{
					if (buffName === MainController.$.model.masterBuffData[i].name)
					{
						...
						addBuff.setMasterData(MainController.$.model.masterBuffData[i], _attackWeaponItem.skill.turn, _attackWeaponItem.skill.level);
						//ターゲットにバフを追加
						_attackWeaponItem.target.buffAdd(addBuff);
						break;
					}
				}
