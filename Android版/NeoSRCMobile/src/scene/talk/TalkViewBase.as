package scene.talk
{
	...
	public class TalkViewBase extends CSprite
	{
		...
		protected function registGenericUnit(name:String, side:String, lv:int, cost:int):void
		{
			...
			{
				if (MainController.$.map.sideState[i].name === name)
				{
					for (j = 0; j < MainController.$.map.sideState[i].genericUnitList.length; j++)
					{
						if (MainController.$.map.sideState[i].genericUnitList[j].compareName(name))
						{
							sideFind = true;
							MainController.$.map.sideState[i].genericUnitList[j] = new GenericUnitData(data, lv, cost);
							break;
						}
					}
				}
