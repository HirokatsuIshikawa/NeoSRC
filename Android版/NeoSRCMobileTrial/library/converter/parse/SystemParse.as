	...
	public class SystemParse
	{
		...
		public static function parseSystenData(str:String):Object
		{
			...
			for (i = 0; i < ary.length; i++)
			{
				...
				else if (line === "インターミッション" || line === "INTERMISSION")
				{
					partseMode = "インターミッション";
					continue;
				}
				...
				else if (partseMode === "インターミッション")
				{
					...
					{
						var pm:Array = param[j].split(":");
						var command:String = pm[0];
						pData[command.toLowerCase()] = pm[1];
					}
					
					if (!pData.hasOwnProperty("title"))
					{
						pData.title = 1;
					}
					
					if (!pData.hasOwnProperty("show"))
					{
						pData.show = 1;
					}
					data.interMission[pData.name] = pData;
				}
			}
			return data;
		}
