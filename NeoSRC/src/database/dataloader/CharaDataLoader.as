package database.dataloader
{
    ...
    public class CharaDataLoader
    {
        ...
        /**軍師データリスト読み込み完了*/
        private function loadCommanderDataListComp(e:Event):void
        {
            var i:int = 0;
            var j:int = 0;
            //イベント消去
            _dataLoader.removeEventListener(BulkLoader.COMPLETE, loadCommanderDataListComp);
            _dataLoader.removeEventListener(BulkLoader.PROGRESS, onProgress);
            _dataLoader.removeEventListener(BulkLoader.ERROR, onError);
            
            MainController.$.view.debugText.addText("軍師データ取得");
            ...
            for (i = 0; i < _dataNameList.length; i++)
            {
                ...
                {
                    data = CommanderDataParse.parseData(_dataLoader.getText(_dataNameList[i]));
                }
