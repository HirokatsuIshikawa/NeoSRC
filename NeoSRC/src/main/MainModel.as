package main
{
    ...
    public class MainModel
    {
        ...
        /**戦闘メッセージリストゲット*/
        public function getCommanderSkillMessageList(keyName:String, commander:CommanderData, state:String, skill:MasterCommanderSkillData, target:BattleUnit = null):Vector.<MessageCondition>
        {
            var list:Vector.<MessageCondition> = new Vector.<MessageCondition>();
            var i:int = 0;
            var j:int = 0;
            ...
            if (list.length <= 0)
            {
                ...
                MainController.$.map.commanderCharaMessage = false;
            }
            else
            {
                MainController.$.map.commanderCharaMessage = true;
            }
        ...
        public function getRandamCommanderSkillMessage(commander:CommanderData,skill:MasterCommanderSkillData, target:BattleUnit):Vector.<String>
        {
            ...
            for (i = 0; i < message.length; i++)
            {
                message[i] = message[i].replace(/{commander}/g, commander.name);
                message[i] = message[i].replace(/{special}/g, skill.name);
                if (target != null)
                {
                    message[i] = message[i].replace(/{target}/g, target.name);
                }
            }
            return message;
        }
